...
const Products = () => {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      axios
        .get(
          `https://mobile-shop-server-weld.vercel.app/allProducts?title=${searchTerm}&sort=${sortOrder}&brand=${brandFilter}&category=${categoryFilter}`
        )
        .then((res) => {
  ...
  const handleAddToCart = (id, email) => {
    fetch(
      `https://mobile-shop-server-weld.vercel.app/addToCart/${id}/${email}`,
      {
        method: "POST",
      }
    )
  ...
  const handleAddToWishList = (id, email) => {
    fetch(
      `https://mobile-shop-server-weld.vercel.app/wishList/${id}/${email}`,
      {
        method: "POST",
      }
    )
            ...
            >
              ...
              {!isAdmin && !isSeller && (
                <div className="flex justify-center gap-4 mt-4">
                  <button
                    onClick={() => handleAddToCart(product?._id, user?.email)}
                  ...
                  <button
                    onClick={() =>
                      handleAddToWishList(product?._id, user?.email)
                    }
